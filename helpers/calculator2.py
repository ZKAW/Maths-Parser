...
def split_expr(expression):
    ...
    pos = 0
    if len(expression) < 1:
        raise TypeError("Expression can't be empty")

    while pos < len(expression):
        char = expression[pos] # Get expression from index
        if char == ' ': pass # Ignore white spaces

        elif char in OPERANDS + PARENTHESES: # If char is a symbol
    ...
    if splited_expr[0] in OPERANDS or splited_expr[-1] in OPERANDS:
        raise SyntaxError("The expression must start and end with a number")
    else:
        return splited_expr
